...
int Arm::power(int speed) {return _max_power_consumed * speed / 100;}
int Arm::max_power() const {return _max_power_consumed;}

ostream& operator<<(ostream& os, const Arm& arm) {
  return os << fixed << setprecision(2)
            << arm.name() << " (" << arm.type() << " PN " << arm.part_number()
            << ") weighs " << arm.weight() << "#, costs $" << arm.cost()
            << ", max power " << arm.max_power() << " watts"
            << " - " << arm.description();
}
